...
test("Throw error when provided invalid parent argument",()=>{
    ...
    expect(function(){mainMenu(document.bodysss,getData);}).toThrow(new Error("First argument isn't an object!"))
})

test("Throw error when provided invalid callback argument",()=>{
    expect(function(){mainMenu(document.body,"getData");}).toThrow(new Error("Callback argument ins't a function!"))
})
