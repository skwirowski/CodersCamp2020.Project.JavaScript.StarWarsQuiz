...
test("Throw error when guessed answer isn't string.",()=>{
    const foo = 2;
    expect(function(){isAnswerCorrect(foo,"answer");}).toThrow(new Error("Guessed answer isn't string"));
})

test("Throw error when correct answer isn't string.",()=>{
    const foo = 2;
    expect(function(){isAnswerCorrect("guess",foo);}).toThrow(new Error("Correct answer isn't string"));
})
